...
export function signNotZero(vector) {
	return new THREE.Vector2(vector.x >= 0 ? 1 : -1, vector.y >= 0 ? 1 : -1);
}

export function decodeOct(encodedVector) {
...
export function constructHighlightedVerticesGeometry(vertexData, indices, container) {
	...
	const positionAttribute = new THREE.BufferAttribute(positionAttributeArray, elementsPerVertex);
...
export function constructPositionAttribute(vertexData, container) {
	const elementsPerVertex = 3;
	const vertexCount = vertexData.length / elementsPerVertex;
...
export function constructNormalAttribute(vertexNormalsBuffer, vertexData) {
	...
	for (
		let position = 0, i = 0;
		position < vertexNormalsBuffer.byteLength;
		position += Uint8Array.BYTES_PER_ELEMENT * elementsPerEncodedNormal, i++
	) {
		const decodedNormal = decodeOct(
			new THREE.Vector2(view.getUint8(position, true), view.getUint8(position + Uint8Array.BYTES_PER_ELEMENT, true))
		);
 ...
 */
 export function constructUvAttribute(verticesArray, container) {
...
export function constructGeometry({ header, vertexData, triangleIndices, extensions }, container) {
...
export function getMeshMaterial(params) {
...
export function getObject(geometry, material, params) {
	switch (params.appearance) {
		case "points": {
...
export function highlightEdgeVertices(decodedMesh, container, scene) {
	const size = 10;
...
export function addWireframe(mesh) {
...
export function addNormalHelper(scene, mesh) {
...
export function createQuantizedMesh(scene, container, params) {
	const containerSize = new THREE.Vector3();
